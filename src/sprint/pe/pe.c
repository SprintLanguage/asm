...
void sprint_writepeexecutable(FILE* file, sprint_bytebuff_t* buff) {
    ...
    fwrite(&dos_header, sizeof(dos_header), 1, file);
    ...
    fwrite(&nt_headers, sizeof(nt_headers), 1, file);
    ...
    fwrite(&section_header, sizeof(section_header), 1, file);
